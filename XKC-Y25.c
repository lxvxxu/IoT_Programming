...
int main(void)
{
  ...
  MX_SPI1_Init();
  /* USER CODE BEGIN 2 */
  printf("Liquid Sensor Test Start (PC6)...\r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // 1. 센서 값 읽기
	      ...
	      // 2. 현재 상태값(0 또는 1)을 무조건 출력해서 확인
	      // NPN 센서라면: 물 감지 시 0, 물 없음 1
	      // PNP 센서라면: 물 감지 시 1, 물 없음 0
	      printf("Sensor Value: %d ", sensorState);

	      if (sensorState == GPIO_PIN_RESET) // 값이 0일 때 (NPN 기준 감지됨)
	      {
	          printf("-> [DETECTED] Liquid Present\r\n");
	          HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_SET); // LED ON
	      }
	      else // 값이 1일 때
	      {
	          printf("-> [EMPTY] No Liquid\r\n");
